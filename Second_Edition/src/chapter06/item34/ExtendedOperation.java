...
public enum ExtendedOperation implements Operation {
    ...
    public static void main(String[] args) {
        ...
        // test2(EnumSet.allOf(BasicOperation.class), x, y);
        test2(Arrays.asList(ExtendedOperation.values()), x, y);

        System.out.println(); // Print a blank line between tests
        HashSet<Operation> operationsAll = new HashSet<>(Arrays.asList(BasicOperation.values()));
        operationsAll.addAll(Arrays.asList(ExtendedOperation.values()));
        test2(operationsAll, x, y);
